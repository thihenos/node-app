$(document).ready(function(){
  ...
  $('.deleteButton').on('click',function(){
    ...
    var url = '/user/'+id; // Creating the URL using the id
    ...
    if(confirm('Are you sure of deleting item '+$(this).data('name')+' ?')){
      $.ajax(
        ...
        //result is a json returned from backend
        //removing
        document.getElementById('divMaterial'+id).remove();
        //rendering the message to the HTML Element
        $('#mensagemRetorno').html(''+result.message);
      });
    }
  });
